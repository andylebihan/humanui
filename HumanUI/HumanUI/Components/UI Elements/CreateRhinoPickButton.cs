...
{
    /// <summary>
    /// Button that triggers a Rhino object pick. The picked GUIDs live on the
    /// HUI_RhPickButton instance; downstream Get-* components read them via
    /// HUI_Util.
    /// </summary>
    public class CreateRhinoPickButton_Component : CreateButton_Component
    {
        ...
        private static readonly ObjectType[] OtMap =
        {
        ...
        protected override void RegisterInputParams(GH_InputParamManager pManager)
        {
            ...
            pManager.AddTextParameter("Prompt", "P", "The prompt to display to the user", GH_ParamAccess.item, "Please select geometry...");
            pManager.AddBooleanParameter("Allow Multiple", "AM", "Set to true to allow selecting multiple items", GH_ParamAccess.item, true);
            pManager.AddBooleanParameter("Allow None", "AN", "Set to true to allow the selection of nothing", GH_ParamAccess.item, true);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (filterInt < 0 || filterInt >= OtMap.Length) filterInt = 0;
            var filter = OtMap[filterInt];

            if (!hasText && !hasIcon) return;

            var btn = new HUI_RhPickButton(prompt, allowMultiple, allowNone, filter);
